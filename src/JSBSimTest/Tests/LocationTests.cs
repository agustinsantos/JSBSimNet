...
namespace JSBSim.Tests
{
    ...
    {
        private const double tolerance = 100.0 * MathExt.DBL_EPSILON; // 10E-15;
        public void CheckLocation(Location loc, Vector3D vec)
        {
            ...
            Assert.AreEqual(vec.Z, loc[3], r * tolerance);
            Assert.AreEqual(r, loc.Radius, r * tolerance);

            vec.Normalize();
            double lon = Math.Atan2(vec.Y, vec.X);
            double lat = Math.Asin(vec.Z);

            Assert.AreEqual(lon, loc.Longitude, tolerance);
            Assert.AreEqual(lat, loc.Latitude, tolerance);
            Assert.AreEqual(Math.Sin(lon), loc.SinLongitude, tolerance);
            Assert.AreEqual(Math.Cos(lon), loc.CosLongitude, tolerance);
            Assert.AreEqual(Math.Sin(lat), loc.SinLatitude, tolerance);
            Assert.AreEqual(Math.Cos(lat), loc.CosLatitude, tolerance);
            Assert.AreEqual(Math.Tan(lat), loc.TanLatitude, tolerance);
        ...
        public void AssertVectorEqual(Vector3D v1, Vector3D v2, double delta)
        {
            Assert.AreEqual(v1.X, v2.X, delta);
            Assert.AreEqual(v1.Y, v2.Y, delta);
            Assert.AreEqual(v1.Z, v2.Z, delta);
        }
        public void AssertMatrixEqual(Matrix3D x, Matrix3D y, double delta = tolerance)
        {
        ...
        public void AssertMatrixIsIdentity(Matrix3D x, double delta)
        {
